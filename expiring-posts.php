...
/*
Plugin Name: Advanced Expiring Posts
...
class EXP_Expiring_Posts {
	...
	function post_meta_box() {
		...
		if ( $can_publish ) : // Contributors don't get to choose the date of publish
			...
			<select style="display:none;">
				<option <?php selected( $post->post_status, 'expired' ); ?> id='expired-status'
				                                                            value='expired'><?php _e( 'Expired' ) ?></option>
			</select>
			<div class="misc-pub-section curtime">
				<span id="exp-timestamp"
				      style="background-image: url(<?php echo admin_url(); ?>images/date-button.gif);"><?php printf( $stamp, $date ); ?></span>
	...
	function select_time( $post_id, $tab_index = 0, $multi = 0 ) {
		...
		<input type="checkbox"
		       name="exp-enable" <?php checked( get_post_meta( $post_id, 'exp_pending_expiration', true ) == false ) ?>
		       id="exp-enable" value="never"/>
